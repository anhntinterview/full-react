...
const ClassList: React.FC = ({ children }) => {
    ...
    // const {
    //     booleanValue: shouldWarningBeforeClose,
    //     toggleBooleanValue: toggleShouldWarningBeforeClose,
    // } = useBoolean();
    const [shouldWarningBeforeClose, setShouldWarningBeforeClose] = useState(
        false
    );

    const [classParams, setClassParams] = useState<IGetClassParams>(
        initClasslistState
    );
    ...
    useEffect(() => {
        workspaceMiddleware.getClassList(dispatch, {
            workspaceId: params.id,
            classParams,
        });
    }, [classParams]);

    const handleSearchClass = (e: string) => {
        setClassParams({
            ...initClasslistState,
            q: e,
        });
    ...
    return (
        <div>
            <div className="w-full h-ooolab_top_sidebar border-b border-ooolab_bar_color py-ooolab_p_2 px-ooolab_p_4">
                {children}
            </div>
            <div className="w-full flex h-ooolab_h_below_15 ">
                <ModalCreate
                    isOpen={modalCreate}
                    onClose={() => {
                        toggleModalCreate();
                    ...
                    shouldWarningBeforeClose={shouldWarningBeforeClose}
                >
                    <CreateClassForm
                        onSuccessCreate={() => {
                        ...
                        // run when form is dirty (touched)
                        ...
                        <div className="flex items-center">
                            ...
                            <div className="h-ooolab_h_8 w-ooolab_w_90 flex flex-row-reverse border border-ooolab_bar_color  text-ooolab_dark_2">
                                <input
                                    ...
                                    onKeyDown={(e: any) => {
                                        if (e.key === 'Enter') {
                                            handleSearchClass(e.target.value);
                                        }
                                    }}
                                />
