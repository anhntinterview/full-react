        ...
        () => [
            {
                Header: ({
                    ...
                    <div className="flex items-center justify-between border-r border-ooolab_bar_color py-ooolab_p_2">
                        <IndeterminateCheckbox
                            {...getToggleAllRowsSelectedProps()}
                        />
                        <RenderGroupsHeader value="Classroom Id" />
                        <span></span>
                        <span></span>
                        <span></span>
                    </div>
                ...
                Cell: ({ row }: { row: Row }) => (
                    <div className="border-r border-ooolab_bar_color flex items-center justify-between h-full py-ooolab_p_4 font-normal">
                        ...
                        />
                        <Link
                            to={`/workspace/${param.id}/management/classroom/${row.values.id}`}
            ...
            },
            {
                Header: () => (
                    <div className="h-full pl-ooolab_p_5 flex items-center">
                        <RenderGroupsHeader value="Classroom Name" />
                    </div>
                ),
                ...
                Cell: ({ row, column }: { row: Row; column: Column }) => {
                    ...
                    return (
                        <EditableCell
                            ...
                            column={column}
                            updateMyData={() => {}}
                            value={row.values.classroom_name}
                        />
                    );
                },
                width: 180,
            },
            {
                Header: () => (
                    <div className="h-full pl-ooolab_p_5 flex items-center">
                        <RenderGroupsHeader value="Group Name" />
                    </div>
                ),
                accessor: 'group_name',
                Cell: ({ row, column }: { row: Row; column: Column }) => {
            ...
            },
            {
                Header: () => (
                    <div className="h-full pl-ooolab_p_5 flex items-center">
                        <RenderGroupsHeader value="Occupancy Size" />
                    </div>
                ),
                accessor: 'size',
                Cell: ({ row, column }: { row: Row; column: Column }) => {
