...
import { mockGroupsData, mockTeacherData } from '../../../components/mock';
...
const TableGroups = () => {
        ...
        () => [
            {
                Header: ({
                    ...
                    <div className="flex items-center justify-between border-r border-ooolab_bar_color py-ooolab_p_2">
                        <IndeterminateCheckbox
                            {...getToggleAllRowsSelectedProps()}
                        />
                        <RenderGroupsHeader value="Teacher ID" />
                        <span></span>
                        <span></span>
                        <span></span>
                    </div>
                ...
                Cell: ({ row }: { row: Row }) => (
                    <div className="border-r border-ooolab_bar_color flex items-center justify-between h-full py-ooolab_p_4 font-normal">
                        ...
                        />
                        <Link
                            to={`/workspace/${param.id}/management/teacher/${row.values.id}`}
            ...
            },
            {
                Header: () => (
                    <div className="h-full pl-ooolab_p_5 flex items-center">
                        <RenderGroupsHeader value="Teacher Name" />
                    </div>
                ),
            ...
            },
            {
                Header: () => (
                    <div className="h-full pl-ooolab_p_5 flex items-center">
                        <RenderGroupsHeader value="Email Address" />
                    </div>
                ),
                ...
                Cell: ({ row, column }: { row: Row; column: Column }) => {
                    ...
                    return (
                        <EditableCell
                            ...
                            column={column}
                            updateMyData={() => {}}
                            value={row.values.email_address}
                        />
                    );
                },
            },
    ...
    return <Table data={mockTeacherData} columns={columns} />;
};
