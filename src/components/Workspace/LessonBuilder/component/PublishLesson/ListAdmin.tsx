...
import { getMemberList } from '../../LessonBuilderFN';
...
type ListAdminPropss = {
    ...
    contentH5P?: IContentListEntry | undefined;
};
...
const ListAdmin: FC<ListAdminPropss> = ({
    close,
    onSubmitApproval,
    contentH5P,
}) => {
    ...
    const { dispatch: dispathH5P, H5PState } = useContext(H5PContext);
    ...
    useEffect(() => {
        setLoading(true);
        getMemberList(dispatch, params.id, {
            role: 'admin',
        });
        return () => {
            setList([]);
    ...
    const onSubmit = () => {
        ...
        if (params.lessonId) {
            onSubmitApproval(translator,dispatch, params.id, params.lessonId, {
                user_id: adminId,
            });
        }
        if (params.contentId || lastItem === 'new') {
            onSubmitApproval(translator,dispathH5P, contentH5P?.argsId, {
                user_id: adminId,
            });
        }
    ...
    return (
        <div className="w-full bg-white p-ooolab_p_4  text-ooolab_dark_1">
            <p className="text-ooolab_lg_e mb-ooolab_m_2">{translator("LESSON.SELECT_ASSIGNEE")}</p>
            ...
            <form onSubmit={handleSubmit(onSubmit)}>
                        ...
                        list.map((i) => (
                            <div key={i.id} className="w-full mb-ooolab_m_4 ">
                                <input
                                    ...
                                    {...register(formName, {
                                        required: `${translator(
                                            'FORM_CONST.REQUIRED_APPROVAL_ASSIGNEE'
                                        )}`,
                                    })}
                                />
                                <label
                ...
                >
                    {translator('LESSON.ASSIGN_APPROVAL')}
                </button>
            </form>
        </div>
    );
