...
const UpdateAvatarForm: React.FC<UpdateAvatarFormProps> = ({
    ...
    function onSave() {
        if (avatar) {
            userMiddlware.uploadAvatarFormData(dispatch, avatar, croppedAvatar);
        }
    }

    // Step 2: upload to AWS
    React.useEffect(() => {
        if (uploadAvatarStateFormDataResult) {
            userMiddlware.uploadAvatarFinal(dispatch, {
                avatar: uploadAvatarStateFormDataResult,
            });
            setSuccessMsg(MESSAGE.UPLOAD_AVATAR_SUCCESS);
        }
        if (uploadAvatarStateAvatarFinalError) {
            setErrorMsg(uploadAvatarStateAvatarFinalError.error.description);
        }
        if (uploadAvatarStateAvatarFinalValidateError) {
            setErrorMsg(
                uploadAvatarStateAvatarFinalValidateError.validation_error
                    .body_params[0].msg
            );
        }
    }, [uploadAvatarStateFormDataResult]);

    // Step 3: save and get avatar_url
    React.useEffect(() => {
        if (uploadAvatarStateAvatarFinalResult) {
            updateLocalStorageUserInfoFields(
                ['avatar_url'],
                [uploadAvatarStateAvatarFinalResult.avatar_url]
            );
            onUpdatedAvatar(uploadAvatarStateAvatarFinalResult.avatar_url);
            userMiddlware.refreshUploadAvatarProcess(dispatch);
        }
    ...
    return (
        <>
            <div className={'space-y-6 flex flex-col'}>
                ...
                >
                    {translator('ACCOUNT_SETTING.CHANGE_PROFILE_PHOTO')}
                </label>
            ...
            {isShowConfirmChangeModal && (
                <Modal
                    ...
                    title={translator(
                        'MODALS.CONFIRM_SAVE_PHOTO_CHANGE_MODAL.TITLE_TEXT'
                    )}
                    imgSrc={ConfirmChanges}
                    contentText={translator(
                        'MODALS.CONFIRM_SAVE_PHOTO_CHANGE_MODAL.CONTENT_TEXT'
                    )}
                    subBtn={
                        <button
                            className={
                        ...
                        >
                            {translator(
                                'MODALS.CONFIRM_SAVE_PHOTO_CHANGE_MODAL.NO_CANCEL'
                            )}
                        </button>
                    }
                    mainBtn={
                        ...
                        >
                            {translator(
                                'MODALS.CONFIRM_SAVE_PHOTO_CHANGE_MODAL.YES_DO_IT'
                            )}
                        </button>
                    }
                />
